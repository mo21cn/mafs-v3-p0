...
def _chain_with_explicit_top1_selection(so: dict, compiled_query: str) -> tuple[dict, dict]:
    ...
    The P1.5-RA3 contract requires explicit candidate selection
    ...
    boundary. This helper preserves the previous end-to-end test
    shape by composing both phases and returning a result that
    exposes the same surface (status, candidate_pointers,

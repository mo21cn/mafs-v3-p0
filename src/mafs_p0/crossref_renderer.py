...
planner. Its sole responsibility is to map an **explicit** compact
search intent (author, year, title, concepts) into Crossref-native
URL parameters, and to expose a small bounded fallback ladder.

P1.5-RA1 (Closure A) removed the prior heuristic intent extractor
that classified domain phrases (e.g. "giant fiber", "hemibrain",
"connectome", "spike-timing", "action selection", "descending",
"sensory-motor") into concepts vs title vs author. The renderer no
longer reads the QueryAST, no longer classifies scientific vocabulary,
and no longer infers scientific intent. The caller MUST supply a
fully-populated ``SearchIntent``; missing fields produce
``rendering_unavailable`` rather than silent fabrication.

The renderer is governed by P1.5 contract sections §3 (What the
Model Owns), §4 (What the Crossref Renderer Owns), §5 (Minimal
Fallback Ladder), §10 (No Generic Query Intelligence Layer), and
P1.5-RA1 §4 (Remove Scientific Intent Inference From the Renderer).
  ...
  - the renderer accepts an explicit ``SearchIntent`` such as
        intent:
          author: "von Reyn"
          year: 2014
  ...
  - each rung preserves the same original search intent; the ladder
    is bounded (3 rungs max + 1 legacy rung for audit);
  - the renderer is not an autonomous research planner;
  - the renderer MUST NOT perform domain-vocabulary recognition,
    scientific concept classification, inferred author / title
    recognition, or any other semantic judgment.

Each rendering is recorded with a stable ``rendering_path`` label
so the orchestrator can audit which rung produced each
candidate set.
"""
...
class SearchIntent:
    ...
    The caller (model / MAFS cognitive layer) is responsible for
    constructing a fully-populated SearchIntent. The renderer will
    NOT classify or extract these fields from any other representation.
    """
